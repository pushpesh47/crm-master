<?php
    ...
    function ___mail_sender($email,$fullname,$template_code,$data) {
            ...
            if(!empty($body) && !empty($email)){
                Mail::send('emails.default', ['body' => $body], function($message) use ($sender){
                    if(!empty($sender['file'])){
                         $message->attach($sender['file'],[ 'mime' => 'application/pdf']);
                    }
                    $message->to(
                        $sender['email'], 
                        $sender['name']
     ...
     function ___sms_sender($data,$otp,$template_code) {
       
        $template = \DB::table('sms')->select(['content','variables'])->where('title',$template_code)->first();
        if(!empty($template)){
            $variables  = explode(',',$template->variables);
            ...
            if(!empty($body) && !empty($data['phone_number'])){

               
                /*$message = rawurlencode($body.' '.$otp);

                $ch = curl_init('http://sms.smartcubes.in/vendorsms/pushsms.aspx?user=shivam01&password=shivam01&msisdn='.$data['phone_number'].'&sid=ALCLIT&msg='.$message.'&fl=0&gwid=2');
                ...
                curl_close($ch);
                 dd($response);*/
                 $api_key = '35D4438D90ECCB';
                //$contacts = '97656XXXXX,97612XXXXX,76012XXXXX,80012XXXXX,89456XXXXX,88010XXXXX,98442XXXXX';
                $from = 'ALCLIT';
                $sms_text = rawurlencode($body.' '.$otp);
                //$sms_text = urlencode('Hello People, have a great day');

                //Submit to server

                $ch = curl_init();
                curl_setopt($ch,CURLOPT_URL, "http://167.86.121.132/app/smsapi/index.php");
                curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
                curl_setopt($ch, CURLOPT_POST, 1);
                curl_setopt($ch, CURLOPT_POSTFIELDS, "key=".$api_key."&campaign=8450&routeid=7&type=text&contacts=".$data['phone_number']."&senderid=".$from."&msg=".$sms_text);
                $response = curl_exec($ch);
                curl_close($ch);
                $response;
                return true;
    ...
    function ___sms_send($data,$template_code) {
        ...
        if(!empty($template)){
            ...
            if(!empty($body) && !empty($data['phone_number'])){
                ...
                /*$ch = curl_init('http://sms.smartcubes.in/vendorsms/pushsms.aspx?user=shivam01&password=shivam01&shivam01='.$data['phone_number'].'&sid=ALCLIT&msg='.$message.'&fl=0&gwid=2');
                curl_setopt($ch, CURLOPT_POST, true);
                curl_setopt($ch, CURLOPT_POSTFIELDS, array());
                curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
                $response = curl_exec($ch);
    ...
    function Pencrypt($plainText,$key)
{
